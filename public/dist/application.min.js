...
!function(app){"use strict";app.registerModule("searches")}(ApplicationConfiguration);
...
"use strict";angular.module("articles").run(["Menus",function(Menus){Menus.addMenuItem("topbar",{title:"Boats",state:"articles",type:"dropdown",roles:["*"]}),Menus.addSubMenuItem("topbar","articles",{title:"Boat Profile",state:"articles.list"}),Menus.addSubMenuItem("topbar","articles",{title:"List Boat",state:"articles.create",roles:["user"]})}]);
...
!function(){"use strict";function menuConfig(Menus){Menus.addMenuItem("topbar",{title:"Searches",state:"searches",type:"dropdown",roles:["*"]}),Menus.addSubMenuItem("topbar","searches",{title:"List Searches",state:"searches.list"}),Menus.addSubMenuItem("topbar","searches",{title:"Create Search",state:"searches.create",roles:["user"]})}angular.module("searches").run(menuConfig),menuConfig.$inject=["Menus"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("searches",{"abstract":!0,url:"/searches",template:"<ui-view/>"}).state("searches.list",{url:"",templateUrl:"modules/searches/client/views/list-searches.client.view.html",controller:"SearchesListController",controllerAs:"vm",data:{pageTitle:"Searches List"}}).state("searches.create",{url:"/create",templateUrl:"modules/searches/client/views/form-search.client.view.html",controller:"SearchesController",controllerAs:"vm",resolve:{searchResolve:newSearch},data:{roles:["user","admin"],pageTitle:"Searches Create"}}).state("searches.edit",{url:"/:searchId/edit",templateUrl:"modules/searches/client/views/form-search.client.view.html",controller:"SearchesController",controllerAs:"vm",resolve:{searchResolve:getSearch},data:{roles:["user","admin"],pageTitle:"Edit Search {{ searchResolve.name }}"}}).state("searches.view",{url:"/:searchId",templateUrl:"modules/searches/client/views/view-search.client.view.html",controller:"SearchesController",controllerAs:"vm",resolve:{searchResolve:getSearch},data:{pageTitle:"Search {{ searchResolve.name }}"}})}function getSearch($stateParams,SearchesService){return SearchesService.get({searchId:$stateParams.searchId}).$promise}function newSearch(SearchesService){return new SearchesService}angular.module("searches").config(routeConfig),routeConfig.$inject=["$stateProvider"],getSearch.$inject=["$stateParams","SearchesService"],newSearch.$inject=["SearchesService"]}();
!function(){"use strict";function SearchesListController(SearchesService){var vm=this;vm.searches=SearchesService.query()}angular.module("searches").controller("SearchesListController",SearchesListController),SearchesListController.$inject=["SearchesService"]}();
!function(){"use strict";function SearchesController($scope,$state,$window,Authentication,search){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.search.$remove($state.go("searches.list"))}function save(isValid){function successCallback(res){$state.go("searches.view",{searchId:res._id})}function errorCallback(res){vm.error=res.data.message}return isValid?void(vm.search._id?vm.search.$update(successCallback,errorCallback):vm.search.$save(successCallback,errorCallback)):($scope.$broadcast("show-errors-check-validity","vm.form.searchForm"),!1)}var vm=this;vm.authentication=Authentication,vm.search=search,vm.error=null,vm.form={},vm.remove=remove,vm.save=save}angular.module("searches").controller("SearchesController",SearchesController),SearchesController.$inject=["$scope","$state","$window","Authentication","searchResolve"]}();
!function(){"use strict";function SearchesService($resource){return $resource("api/searches/:searchId",{searchId:"@_id"},{update:{method:"PUT"}})}angular.module("searches").factory("SearchesService",SearchesService),SearchesService.$inject=["$resource"]}();
